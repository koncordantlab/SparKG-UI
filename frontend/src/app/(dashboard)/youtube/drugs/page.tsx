...
export default function YouTubeDrugsPage() {
  ...
  const loadDrugs = async () => {
    try {
      setLoading(true)
      // Load all drugs with YouTube mentions
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">YouTube Drug List</h1>
        <p className="text-gray-500">Drugs monitored on YouTube with activity stats</p>
      </div>
            ...
            {drugFilterDropdownOpen && (
              <div className="absolute z-50 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-80 overflow-hidden">
                ...
                <div className="max-h-60 overflow-y-auto">
                  <div
                    className={`px-3 py-2 cursor-pointer hover:bg-gray-100 ${!selectedDrugFilter ? 'bg-red-50 text-red-700' : ''}`}
                  ...
                  {filteredDrugOptions.slice(0, 50).map(drug => (
                    <div
                      ...
                      className={`px-3 py-2 cursor-pointer hover:bg-gray-100 ${selectedDrugFilter === drug.scientific_name ? 'bg-red-50 text-red-700' : ''}`}
          ...
          <div className="relative" ref={categoryDropdownRef}>
            ...
            {categoryDropdownOpen && (
              <div className="absolute z-50 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-80 overflow-hidden">
                ...
                <div className="max-h-60 overflow-y-auto">
                  <div
                    className={`px-3 py-2 cursor-pointer hover:bg-gray-100 ${!selectedCategory ? 'bg-red-50 text-red-700' : ''}`}
                  ...
                  {filteredCategories.map(cat => (
                    <div
                      key={cat.category}
                      className={`px-3 py-2 cursor-pointer hover:bg-gray-100 ${selectedCategory === cat.category ? 'bg-red-50 text-red-700' : ''}`}
              ...
              {filteredDrugs.map((drug) => (
                <div
                  ...
                  className={`bg-white rounded-xl shadow-sm p-4 cursor-pointer border hover:shadow-md transition-shadow ${
                    selectedDrug?.scientific_name === drug.scientific_name ? 'ring-2 ring-red-500' : ''
                  }`}
                >
        ...
        {selectedDrug && (
          <div className="w-80 bg-white rounded-xl shadow-sm p-6 h-fit sticky top-6 border">
            ...
            {loadingStats ? (
              ...
              <>
                <h4 className="text-sm font-medium text-gray-700 mb-3">YouTube Activity</h4>
                <div className="space-y-3">
                  <div className="bg-slate-50 rounded-lg p-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">Total Videos</span>
                      <span className="text-lg font-bold text-slate-700">
                        {formatNumber(selectedDrug.youtube_videos)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 rounded-lg p-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-bold text-slate-700">
                        {formatNumber(selectedDrug.youtube_views)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 rounded-lg p-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-bold text-slate-700">
                        {formatNumber(selectedDrug.youtube_likes)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <Link
                  href={`/youtube/videos?drug=${encodeURIComponent(selectedDrug.scientific_name)}`}
